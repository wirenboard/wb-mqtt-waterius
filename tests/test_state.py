    ...
    state.save_state(
        {
            ...
            "schedule_time": "12:00",
            "last_sent": {"a1b2c3d4e5f6": "2026-07-16 12:00:00"},
        }
    )
    loaded = state.load_state()
    assert loaded["enabled"] is False
    assert loaded["last_sent_date"] == "2026-07-16"
    assert loaded["schedule_time"] == "12:00"
    assert loaded["last_sent"] == {"a1b2c3d4e5f6": "2026-07-16 12:00:00"}
...
def test_load_state_defaults_on_bad_input(content: Optional[str], tmp_path: Path) -> None:
    ...
    assert loaded["last_sent_date"] is None
    assert loaded["last_sent"] == {}
...
def test_save_state_atomic_leaves_valid_file_no_tmp(tmp_path: Path) -> None:
    state.save_state(
        {"enabled": False, "last_sent_date": "2026-07-16", "schedule_time": "12:00", "last_sent": {}}
    )
